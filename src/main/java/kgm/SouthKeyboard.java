 ...
 * @author Naruto
 */
public class SouthKeyboard extends JPanel {
    
    private int octaveNumber = 1;
    private Octave firstOctave = Octave.Octave04;
    private final List<Mini> miniList = new ArrayList<>();
    
    public SouthKeyboard(int width, int height) {
        init(width, height);
    }
    
    public SouthKeyboard(int width, int height, int octaveNumber, Octave firstOctave) {
    ...
    private void init(int width, int height){
        ...
        for(int i=0; i<octaveNumber; i++){
            Mini m = new Mini(compSizeX, compSizeY, Direction.South, next);
            m.setLocation(compSizeX*i, 0);
            miniList.add(m);
            this.add(m);
