 ...
 * @author Naruto
 */
public class WestKeyboard extends JPanel {
    
    private int octaveNumber = 1;
    private Octave firstOctave = Octave.Octave04;
    private final List<Mini> miniList = new ArrayList<>();
    
    public WestKeyboard(int width, int height) {
        init(width, height);
    }
    
    public WestKeyboard(int width, int height, int octaveNumber, Octave firstOctave) {
    ...
    private void init(int width, int height){
        ...
        for(int i=0; i<octaveNumber; i++){
            Mini m = new Mini(compSizeX, compSizeY, Direction.West, next);
            m.setLocation(0, height - compSizeY*(i+1));
            miniList.add(m);
            this.add(m);
